...
RSpec.feature 'authenticated user can view previous orders' do
  ...
  # And when I click that link
    click_on (href: order_path(order)) #need to figure out how to shovel item into order
  ...
  # And I should see links to each item's show page
    expect(page).to have_link('Ice', href: item_path(item1))
    expect(page).to have_link('Ice2', href: item_path(item2))
